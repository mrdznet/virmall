...
class Styles extends Base{
	...
	public function initStyles(){
		...
		foreach ($styles as $key => $v) {
			...
			//删除不存在的风格记录
			if(!is_dir(WSTRootPath(). DS .'application'. DS .$v['styleSys']. DS .'view'. DS.$v['stylePath'])){
        ...
        try{
			...
			foreach ($sys as $key => $v) {
			    $dirs = array_map('basename',glob(WSTRootPath(). DS .'application'.DS.$key.DS.'view'.DS.'*', GLOB_ONLYDIR));
		        foreach ($dirs as $dkey => $dv) {
		        	if(!isset($v[$dv])){
		        		$sqlPath = WSTRootPath(). DS .'application'. DS .$key. DS .'view'. DS .$dv. DS.'sql'.DS.'install.sql';// sql路径
	...
	public function changeStyle(){
		 ...
		 $sqlPath = WSTRootPath(). DS .'application'. DS .$object['styleSys']. DS .'view'. DS .$object['stylePath']. DS.'sql'.DS.'style.sql';// sql路径
		 $hasFile = file_exists($sqlPath);
